...
-- Fast vector search for candidates, then rerank
-- ============================================================================
...
WITH Candidates AS (
    SELECT TOP 25
        ...
        e.video_id,
        ec.start_seconds,
        CAST(VECTOR_DISTANCE('cosine', ce.embedding, @query) AS DECIMAL(5,4)) AS vector_distance,
        ROW_NUMBER() OVER (ORDER BY VECTOR_DISTANCE('cosine', ce.embedding, @query)) AS vector_rank
    FROM dbo.ChunkEmbeddings ce
    JOIN dbo.EpisodeChunks ec ON ce.chunk_id = ec.chunk_id
    JOIN dbo.Episodes e ON ec.episode_id = e.episode_id
...
Reranked AS (
    SELECT *,
        -- Simulate reranking: boost if keywords appear
        CASE
            WHEN speaker_answer LIKE '%feedback%' AND speaker_answer LIKE '%relationship%'
                THEN vector_distance * 0.8
            WHEN speaker_answer LIKE '%feedback%' OR speaker_answer LIKE '%tough%'
                THEN vector_distance * 0.9
...
SELECT TOP 10
    ...
    vector_rank,
    vector_distance,
    CAST(rerank_score AS DECIMAL(5,4)) AS rerank_score
FROM Reranked
ORDER BY rerank_score;
GO

-- ============================================================================
-- 6. COMPARE PERSPECTIVES
